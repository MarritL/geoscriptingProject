...
environmentalURL = "https://drive.google.com/uc?export=download&id=16IRkTgB5v0iZFPaJI_DnVSDeGYrbnHTL"
...
The height measurement of the GPS data needs to be validated through a comparison with the DEM. GPS data are not reliable if its mean height measurements differ significantly from the DEM values at corresponding spatial points. A paired t-test is performed to assess the reliability the height measurements.
```{r}
# Calculate DEM height values at point locations for which GPS height data is available.
heightDEM <- extract(DEM, as.data.frame(groupsCleaned[,1:2]), method='simple', na.rm=TRUE, df=TRUE)

heightData <- data.frame(group = rep(c("DEM", "GPS"), each = 10974), height = c(heightDEM$DEM_be75, groupsCleaned$height))

# Perform a paired t-test to compare mean GPS and DEM height
tTest <- t.test(heightDEM$DEM_be75, groupsCleaned$height, paired = TRUE, alternative = "two.sided")
...
The paired t-test confirmed that there is a significant difference (p-value < 2.2e-16) in mean heights between the GPS height measurement and the DEM height values. The mean difference between the two height measurements amounts to 41 meters. As shown in the boxplot, the GPS data contains large outliers (the alps are not 20 000 in height!). This confirms our suspicion that the GPS height measurements are not always reliable. Therefore, we will use DEM height measurement for our further analysis.  
...
```{r}
#beestjes aan en uit
#achtergrond varieren?
#inzoomen met grotere scope

#epsg32632 <- leafletCRS(crsClass = "L.Proj.CRS", code = "EPSG:332632",
#  proj4def = "+proj=utm +zone=32 +datum=WGS84 +units=m +no_defs",
#  resolutions = 2^(13:-1), # 8192 down to 0.5
#  origin = c(0, 0)
#)
#m <- leaflet#(options = leafletOptions(worldCopyJump = F, crs = epsg32632))
...
########### check if new dem etc. are correct and clip to extent. check if you still get same error non no-missing args. Add layers in leaflet to switch mvoement per animal on and off etc.#########
crs(landUse) <- sp::CRS("+proj=merc +a=6378137 +b=6378137 +lat_ts=0.0 +lon_0=0.0 +x_0=0.0 +y_0=0 +k=1.0 +units=m +nadgrids=@null +wktext  +no_defs")
test <- leaflet() %>% 
  addTiles() %>% 
  setView(lng=11.0367, lat=46.0331, zoom = 11) 
test <- addRasterImage(test, landUse) 
test
...
m <- leaflet() %>%
  ...
  setView(lng=11.0367, lat=46.0331, zoom = 11) %>% 
  addProviderTiles("CartoDB.PositronOnlyLabels") %>% 
  addMarkers(data = head(selectedMovementData, n=1000))
  #addMarkers(data = dfPoints)
m  # Print the map
```

```{r}
# create a normal dataframe with cleaned data
dataframeCleaned <- ungroup(groupsCleaned)

coords <- dataframeCleaned[,c(1:2)]
zone32cleanedData <- SpatialPointsDataFrame(coords = coords, data = dataframeCleaned, proj4string = CRS(proj4zone32))
WGS84cleanedData <- spTransform(zone32cleanedData, CRS(proj4WGS84))



# from the sp vignette:
l1 = cbind(c(659213.4, 659189.9, 659058.3, 659281.3, 659181.9, 659185.8),c(5818582, 5818585, 5818590, 5818585, 5818597, 5818589))
rownames(l1) = letters[1:6]
l1a = cbind(l1[,1]+.05,l1[,2]+.05)
rownames(l1a) = letters[1:6]
...
Sl = SpatialLines(list(S1), proj4string = CRS(proj4WGS84))
summary(Sl)
