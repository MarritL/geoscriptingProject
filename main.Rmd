...
For visualization purposes we need to create Spatial Lines of the recorded trajectories.
...
For visualization purposes we now need to create Spatial Lines of the recorded trajectories without outliers.
...
m 
```
Note! The data might still contain a few wrongly recorded points. For example, we are not sure whether Agostino really visited his aunt in Stravino for a coffee. However, we are not justified to remove these potential outliers on the basis of our previously defined cut-off point. 
...
# initialize an empty dataframe. Loop over rec and put into column. Loop over sim and put into seperate column.
# indexing in loop. 

df <- data.frame(meanrec = c(0, 0, 0, 0, 0))
                 
  for (animal in summaryStats){
    noSim <- ncol(animal)-1
    recordedmean <- summaryStats[[animal]][1,1]
    simulatedmeans <-  summaryStats[[animal]][1,2:ncol(summaryStats[[1]])]
  }
df[,1] <- recordedmean
df[,2] <- simulatedmeans


meanrec1 <- summaryStats[[1]][1,1]
meanrec2 <- summaryStats[[2]][1,1]
meanrec3 <- summaryStats[[3]][1,1]
meanrec4 <- summaryStats[[4]][1,1]
meanrec5 <- summaryStats[[5]][1,1]

meansim1 <- summaryStats[[1]][1,2:ncol(summaryStats[[1]])]
meansim2 <- summaryStats[[2]][1,2:ncol(summaryStats[[2]])]
meansim3 <- summaryStats[[3]][1,2:ncol(summaryStats[[3]])]
meansim4 <- summaryStats[[4]][1,2:ncol(summaryStats[[4]])]
meansim5 <- summaryStats[[5]][1,2:ncol(summaryStats[[5]])]
meansrec <- c(meanrec1, meanrec2, meanrec3, meanrec4, meanrec5)
meanssim <- c(meansim1, meansim2, meansim3, meansim4, meansim5)

plot(meansrec, col = c('red'))
plot(meanssim, col = c('grey'), add = TRUE)
...
grid.arrange(h1,h2,h3,h4,h5, layout_matrix = rbind(c(1,2),c(3,4),c(5,6)))
