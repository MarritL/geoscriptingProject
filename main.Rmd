...
```{r}
#Extract hoogte uit DEM met punten van simulaties (simulations). 

for (animal in length(simulations)){
  for (nSim in length(simulations[[animal]])){
    traject <- simulations[[animal]][[nSim]][[1]]
    
    heightSim <- extract(DEM, traject[,c("x","y")], method='simple', na.rm=TRUE, df=TRUE)
    heightSim$nSim <- nSim
    heightSim$animal <- id(simulations[[animal]][[nSim]])
  }
}
...
# Van trajectories naar dataframe: ld.
