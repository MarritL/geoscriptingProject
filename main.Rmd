...
legendFile <- list.files(path = folder, pattern = glob2rx('*.csv'), full.names = TRUE)
landUseLegend <- read.csv(legendFile)
```
...
boxplot.matrix(simElCombi[[1]], use.cols = TRUE)
DFsimElCombi <- stack(as.data.frame(simElCombi[[1]])) # add colnames
#colnames(test3) <- c("")
...
ggplot(DFsimElCombi) +
  ...
  xlab("Trajectory") +
  ylab("Elevation (m)") +
  ggtitle("Comparison of recorded and simulated GPS trajectories")
...
par(mfrow=c(2, 2))
#hist(simLandCombi[[1]][,1], probability = TRUE, breaks = seq(from 0, by = 1, length.out = ))
hist(simLandCombi[[1]][,2], probability = TRUE)
hist(simLandCombi[[1]][,3], probability = TRUE)
par(mfrow=c(1, 1))
```
