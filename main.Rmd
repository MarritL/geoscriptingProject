...
```{r}
# variables
animalName <- id(movementTrajectories)
startdate <- summarise(groupsCleaned, startdate = min(date))[,'startdate'][[1]]
firstRow <- groupsCleaned %>% filter(row_number()==1)
startcoords <- as.data.frame(firstRow[,1:2])
nSteps <- group_size(groupsCleaned)
timelag <- summarize(groupsCleaned, timelag = mean(dt))[,'timelag'][[1]] # mean time between steps in seconds

# create dateseq
dateSeq <- seq(from = startdate[1], by= timelag[1], length.out = nSteps[1])

# one simulation and plot
sim1 <- simm.crw(dateSeq, h = 1.1, r = 0, x0 = c(658992.4, 5097715), id = animalName[1], proj4string = CRS(proj4zone32))
mean(sim1[[1]]$dist, na.rm=T)
plot(DEM)
lines(sim1[[1]])
...
dfPoints <- as.data.frame(groupsCleaned[,1:2])
...
crs(DEM) <- sp::CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs")
