...
for (animal in 1:length(animalNames)){
  ...
  assign(paste0("h",animal),ggplot(data=groupedLandUseCount, aes(x=LABEL2, y=count, fill=ind)) +
    geom_bar(stat="identity", position=position_dodge()) + ggtitle(paste("Land use of", animalNames[animal], "and simulations")) + xlab(NULL) + theme(axis.text.x = element_text(angle = 15, hjust = 1)))
}
