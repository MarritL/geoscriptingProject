...
```{r}
myTheme2=rasterTheme(region=rev(brewer.pal('Paired', n=9)))
levelplot(landUse, main = "Landuse at recorded and simulated roe deer locations", margin = FALSE, par.settings=myTheme2,
          xlim=c(640000, 680000))

landUseRat <- ratify(landUse)
rat <- levels(landUseRat)
landUseRat$LandUse_2012 <- landUseLegend$LABEL2
levels2 <- landUseLegend$LABEL2
landUseRat$LandUse_2012 <- levels()
levels(landUseRat) <- rat
...
rat2drop <- rat2[,c("ID","LABEL3")]

levels(landUseTest) <- rat2drop


levelplot(landUseTest, col.regions=c('palegreen', 'midnightblue', 'indianred1'))

levelplot(landUseTest, col.regions = brewer.pal('Paired'))
levelplot(r, main = "Altitude of recorded roe deer locations", margin = FALSE, par.settings=myTheme, xlim = c(647000, 670000))
levelplot(landUseTest, main = "Altitude of recorded roe deer locations", margin = FALSE, par.settings=myTheme, xlim = c(647000, 670000))
#
