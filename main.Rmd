...
colnames(heightDEM) <- c("ID", "elevation", "animal")

heightData <- data.frame(group = rep(c("DEM", "GPS"), each = nrow(heightDEM)), height = c(heightDEM$elevation, groupsCleaned$height))

# Perform a paired t-test to compare mean GPS and DEM height
tTest <- t.test(heightDEM$elevation, groupsCleaned$height, paired = TRUE, alternative = "two.sided")
...
```{r}
# per dier
#simElAgostino <- simulationElevations[simulationElevations$animal == 'Agostino',]
#DEMElAgostino <- heightDEM[heightDEM$animal == 'Agostino',]

#boxplot(height ~ group, heightData, main = "Height boxplot GPS and DEM", ylab = "Height")

#boxEl <- ggplot(simulationElevations$rasterVal, aes(x=id, y=x, fill = id)) + geom_boxplot(alpha=0.2) + #theme(legend.position = "none") + ggtitle("x-coordinates of locations")
...
#data.frame(group = rep(c("DEM", "GPS"), each = nrow(heightDEM)), height = c(heightDEM$elevation, groupsCleaned$height))

# <- dataframeTrajectories %>% mutate(height = movementSDF$height) %>% group_by(id)
#trajectories <- dl(groups)
...
# second order polynomial (negative)
# entropy of a series of bits
# run.tests
```
...
print(paste("The entropy of the recorded altitude for", as.character(simAgostino$animal[1]), "is: ", entropyDEMAgostino))
