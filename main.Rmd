...
m <- leaflet() %>% 
  ...
  # Overlay groups
  addMarkers(data = head(selectedMovementData, n=1000), group = 'Agostino') %>% 
  addMarkers(data = head(selectedMovementData, n=1000), group = 'Allesandra') %>% 
  addLayersControl(
    baseGroups = c('Street map', 'Terrain', 'Satellite'),
    overlayGroups = c('Agostino', 'Alessandra'),
