...
environmentalURL = "https://drive.google.com/uc?export=download&id=1WpYQrWbM1JIChZXOYkSnrR8HOHl9XnRZ"
...
m <- leaflet() %>%
  ...
  addProviderTiles('Stamen.TopOSMRelief') %>% 
  setView(lng=11.0367, lat=46.0331, zoom = 11) #%>% 
...
```{r}
# create spatial lines between the animal locations

# select per animal
animal1 <- groupsCleaned %>% filter(id == "Agostino")
coords <- animal1[,c("x","y")]
zone32animal1 <- SpatialPointsDataFrame(coords = coords, data = animal1, proj4string = CRS(proj4zone32))
mercatorAnimal1 <- spTransform(zone32animal1, CRS(proj4WGS84))
longitude <- (mercatorAnimal1@coords[,"x"])
latitude <- (mercatorAnimal1@coords[,"y"])
l1 = cbind(longitude,latitude)
Sl1 = Line(l1)
```
